...
func newLogger(traceID ...string) Logger {
	...
	if config.Config.Logger.ConsoleLoggingEnabled {
		if config.Config.Logger.EncodeLogsAsJSON {
	...
	if config.Config.Logger.FileLoggingEnabled {
		if config.Config.Logger.EncodeLogsAsJSON {
			writers = append(writers, config.Config.Logger.RollingWrite)
		} else {
			writers = append(writers, zerolog.ConsoleWriter{Out: config.Config.Logger.RollingWrite, TimeFormat: DefaultLogTimeFormat})
		}
	}
	...
	if config.Config.Logger.LogLevel != 0 {
		DefaultLogLevel = Level(config.Config.Logger.LogLevel)
	}
